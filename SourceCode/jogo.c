...
void passar_bastao(int id) {
    ...
    enviar_mensagem((char) MEN_BASTAO, (char) computador.id, computador.confirm_biometria, (char)id, 0);
...
void verifica_pulos_seguidos() {
    ...
    if (jogo.contador_pulos >= computador.qtd_maquinas) {
        ...
        enviar_mensagem((char) MEN_RODADA_ACABOU, (char) computador.id, computador.confirm_biometria, 0, 0);
        protocolo_de_tratamento();
...
void decreta_vitoria() {
    ...
    enviar_mensagem((char) MEN_JOGADOR_VENCEU, (char) computador.id, computador.confirm_biometria, 0, 0);
    protocolo_de_tratamento();
}
...
int loop_jogo() {
    ...
    enviar_mensagem((char) MEN_CONEXAO, computador.id, computador.confirm_biometria, 0, 0);
    ...
    while(jogo.estado_jogo == JOGO_ESTADO_COMPRANDO) {
        ...
        if (jogo.bastao) {
            ...
            enviar_mensagem((char) MEN_JOGO_INI, computador.id, computador.confirm_biometria, 0, 0);
            protocolo_de_tratamento();
        }
    }
    ...
    while(jogo.estado_jogo == JOGO_ESTADO_INICIADO) {
        ...
        if (jogo.bastao) {
            ...
            switch(input) {
                ...
                case(2) :   // Pula sua vez
                    enviar_mensagem((char) MEN_PULANDO, (char)computador.id, computador.confirm_biometria, 0, 0);
                    protocolo_de_tratamento();
                    passar_bastao(computador.id_next);
                break;
